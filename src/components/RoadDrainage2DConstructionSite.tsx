...
type RoadDrainage2DConstructionSiteProps = {
  buildStage: number;
  feedback?: string;
};

const messages = [
  'Answer the first maths question to begin the road drainage upgrade.',
  ...
  'Concrete gutter walls are built on both sides of the road.',
  'Road drainage upgrade complete. Click Test Heavy Rain to check the system.'
];
...
function Car({ x, y, color }: { x: number | string; y: number | string; color: string }) {
  return (
    <g transform={`translate(${x} ${y})`} filter="url(#drainShadow)">
...
export default function RoadDrainage2DConstructionSite({ buildStage, feedback }: RoadDrainage2DConstructionSiteProps) {
  ...
  return (
    <div className="road-drain-site" aria-label={`Road drainage upgrade stage ${stage} of 6`}>
      <div className="road-drain-site__header">
        <div>
          <p className="road-drain-site__eyebrow">Rain-Safe Road & Drainage Upgrade</p>
          <h2>Raise the Road and Upgrade the Gutters</h2>
          <p>{visibleFeedback}</p>
        </div>
        <div className="road-drain-site__status">Stage {stage} of 6</div>
      </div>

      <div className="road-drain-site__canvas">
        <svg viewBox="0 0 980 620" role="img" aria-label="Community road drainage upgrade simulation">
          ...
          `}</style>
          <defs>
            <linearGradient id="roadDrainSky" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#94a3b8" /><stop offset="58%" stopColor="#cbd5e1" /><stop offset="100%" stopColor="#dcfce7" /></linearGradient>
            <linearGradient id="roadDrainAsphalt" x1="0" x2="0" y1="0" y2="1"><stop offset="0%" stopColor="#64748b" /><stop offset="100%" stopColor="#1f2937" /></linearGradient>
            <linearGradient id="roadDrainWater" x1="0" x2="1"><stop offset="0%" stopColor="#0ea5e9" /><stop offset="100%" stopColor="#0284c7" /></linearGradient>
            <filter id="drainShadow" x="-25%" y="-25%" width="150%" height="150%"><feDropShadow dx="0" dy="8" stdDeviation="7" floodColor="#0f172a" floodOpacity="0.24" /></filter>
          </defs>
          <rect width="980" height="620" fill="url(#roadDrainSky)" />
          ...
          <g filter="url(#drainShadow)">
            <rect x="40" y={roadY} width="900" height={roadHeight} rx="16" fill="url(#roadDrainAsphalt)" />
          ...
          {stage === 0 && <g><path d="M58 276 C230 330 390 252 520 310 C650 368 820 286 930 328 L930 418 C760 390 618 448 480 390 C320 324 184 406 58 360 Z" fill="#38bdf8" opacity=".58" /><path d="M92 404 C240 378 318 438 472 416 C628 394 742 446 910 410" fill="none" stroke="#e0f2fe" strokeWidth="8" strokeDasharray="28 16" opacity=".85" className="flow" /><text x="490" y="464" textAnchor="middle" fill="#075985" fontSize="24" fontWeight="900">shallow gutters overflow onto the road</text></g>}
          ...
          {stage >= 3 && <g className="pop" filter="url(#drainShadow)"><rect x="46" y="218" width="98" height="134" rx="12" fill="#451a03" /><rect x="836" y="218" width="98" height="134" rx="12" fill="#451a03" /><text x="490" y="206" textAnchor="middle" fill="#7c2d12" fontSize="20" fontWeight="900">deep gutters: 100cm × 90cm = 9000cm²</text></g>}
          {stage >= 5 && <g className="pop" filter="url(#drainShadow)"><rect x="46" y="218" width="98" height="134" rx="12" fill="#94a3b8" stroke="#475569" strokeWidth="5" /><rect x="836" y="218" width="98" height="134" rx="12" fill="#94a3b8" stroke="#475569" strokeWidth="5" /><rect x="64" y="236" width="62" height="98" rx="8" fill="url(#roadDrainWater)" /><rect x="854" y="236" width="62" height="98" rx="8" fill="url(#roadDrainWater)" /><path d="M68 256 C86 244 104 270 124 256" fill="none" stroke="#e0f2fe" strokeWidth="5" className="flow" /><path d="M858 256 C876 244 894 270 914 256" fill="none" stroke="#e0f2fe" strokeWidth="5" className="flow" />{stage >= 6 && <g>{[58,86,114,858,886,914].map((x) => <rect key={x} x={x} y="210" width="14" height="146" rx="5" fill="#475569" opacity=".78" />)}<rect x="184" y="198" width="196" height="54" rx="12" fill="#dcfce7" stroke="#16a34a" strokeWidth="4" /><text x="282" y="232" textAnchor="middle" fill="#166534" fontSize="18" fontWeight="900">RAIN-SAFE ROAD</text></g>}</g>}
          ...
          <circle cx="380" cy="502" r="10" fill="#7c2d12" /><rect x="372" y="512" width="16" height="30" rx="6" fill="#2563eb" /><circle cx="612" cy="502" r="10" fill="#7c2d12" /><rect x="604" y="512" width="16" height="30" rx="6" fill="#2563eb" />
      ...
      {stage >= 6 && <button className="road-drain-site__test" type="button" onClick={() => setTesting((current) => !current)}>{testing ? 'Reset Heavy Rain Test' : 'Test Heavy Rain'}</button>}
      <style>{`.road-drain-site{font-family:Inter,system-ui,sans-serif;color:#0f172a}.road-drain-site__header{display:flex;justify-content:space-between;align-items:flex-start;gap:1rem;margin-bottom:1rem}.road-drain-site__eyebrow{margin:0 0 .25rem;color:#0369a1;font-weight:900;text-transform:uppercase;letter-spacing:.08em;font-size:.82rem}.road-drain-site h2{margin:.1rem 0 .35rem;font-size:clamp(1.3rem,3vw,2.1rem)}.road-drain-site p{margin:0;font-weight:700;color:#334155}.road-drain-site__status{background:#e0f2fe;border:2px solid #38bdf8;border-radius:999px;padding:.7rem 1rem;font-weight:900;color:#075985;white-space:nowrap}.road-drain-site__canvas{border-radius:24px;overflow:hidden;border:4px solid #bae6fd;background:#e0f2fe;box-shadow:inset 0 0 0 1px rgba(15,23,42,.08)}.road-drain-site svg{display:block;width:100%;height:auto;min-height:360px}.road-drain-site__test{margin-top:1rem;width:100%;border:0;border-radius:18px;background:linear-gradient(135deg,#0284c7,#16a34a);color:#fff;font-weight:1000;font-size:1rem;padding:1rem 1.25rem;cursor:pointer;box-shadow:0 14px 30px rgba(2,132,199,.25)}.road-drain-site__test:hover{filter:brightness(1.05);transform:translateY(-1px)}@media(max-width:720px){.road-drain-site__header{flex-direction:column}.road-drain-site__status{border-radius:16px}.road-drain-site svg{min-height:300px}}`}</style>
    </div>
  );
}
