...
function Reveal({ show, children }: { show: boolean; children: ReactNode }) {
  ...
  useFrame((_, delta) => {
    ...
    ref.current.position.y = (1 - progress.current) * 0.18;
  });
...
function RiggedLearnerAvatar({ position, walking, cheering, shirt }: { position: Vec3; walking: boolean; cheering: boolean; shirt: string }) {
  ...
  useEffect(() => {
    const actionName = walking ? 'Walk' : 'Idle';
...
function TestingLearners({ visible, testing, resetKey, crossed, onDone }: { visible: boolean; testing: boolean; resetKey: number; crossed: boolean; onDone: () => void }) {
  ...
  useFrame((state, delta) => {
    ...
    groupRefs.current.forEach((group, index) => {
      if (!group) return;
      const local = elapsed.current - index * 3.15;
      const t = THREE.MathUtils.clamp(local / 2.85, 0, 1);
      ...
      if (t <= 0.18) {
        ...
        group.position.y = THREE.MathUtils.lerp(0.04, 1.12, p);
      } else if (t <= 0.78) {
        const p = (t - 0.18) / 0.6;
        group.position.x = THREE.MathUtils.lerp(-2.55, 2.55, p);
        group.position.y = 1.12 + Math.sin(state.clock.elapsedTime * 8 + index) * 0.018;
      } else {
        const p = (t - 0.78) / 0.22;
        group.position.x = THREE.MathUtils.lerp(2.55, 3.35 + index * 0.18, p);
        group.position.y = THREE.MathUtils.lerp(1.12, 0.04, p);
      }
      group.position.z = lanes[index];
      group.rotation.y = 0;

      if (eased === 0) group.position.x = -3.35;
    });

    if (allDone && elapsed.current > 11.4) {
      done.current = true;
      onDone();
    }
  });
  ...
  return (
    <group>
      {lanes.map((z, index) => {
        const active = testing && elapsed.current >= index * 3.15 && elapsed.current < index * 3.15 + 2.85;
        const completed = crossed || (testing && elapsed.current >= index * 3.15 + 2.85);
        return (
          <group
            key={z}
            ref={(node) => {
          ...
          >
            <LearnerAvatar position={[0, 0, 0]} walking={active} cheering={completed} shirt={shirts[index]} />
          </group>
        );
      })}
      {crossed && (
  ...
  return (
    <section className={`bridge-3d-shell ${fullscreen ? 'is-fullscreen' : ''}`} aria-label="Animated 3D bridge builder scene">
      ...
      <style>{`
        ...
        .bridge-3d-canvas-wrap{min-height:360px;height:min(41vw,500px);max-height:540px;width:100%;background:linear-gradient(180deg,#dbeafe,#f0f9ff)}.bridge-3d-canvas-wrap canvas{display:block;width:100%!important;height:100%!important}
